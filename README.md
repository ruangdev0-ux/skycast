...
[![Deploy](https://github.com/ruangdev0-ux/conectaedu/actions/workflows/deploy.yml/badge.svg)](https://github.com/ruangdev0-ux/conectaedu/actions/workflows/deploy.yml)

Aplicação web que mostra as condições atuais e a previsão de 7 dias para qualquer cidade do mundo, usando a API gratuita [Open-Meteo](https://open-meteo.com/) (sem necessidade de chave de API).

**Demonstração ao vivo:** https://ruangdev0-ux.github.io/conectaedu/

![Painel do SkyCast exibindo a previsão para São Paulo](docs/dashboard.png)

## Funcionalidades

- Busca por nome de cidade (API de geocodificação do Open-Meteo).
...
- Mensagens de erro claras: cidade não encontrada, API indisponível e permissão de localização negada.
...
- [Vite](https://vitejs.dev/) 5 como ferramenta de build e servidor de desenvolvimento
...
```bash
git clone https://github.com/ruangdev0-ux/conectaedu.git
cd conectaedu
npm install
npm run dev
```
...
A cada push na branch `main`, um workflow do GitHub Actions instala as dependências, executa `npm run build` e publica a pasta `dist/` no GitHub Pages.
...
Ruan Gomes — [@ruangdev0-ux](https://github.com/ruangdev0-ux)
